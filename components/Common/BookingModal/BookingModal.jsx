...
export default function BookingModal({ isOpen, onClose }) {
  ...
  // Drives a genuinely different (not just resized) markup tree below
  // 769px - see the mobile branch in the JSX further down. Same breakpoint
  // BookingModal.css historically used for its resized-desktop mobile
  // rules, now expressed as a real layout switch instead of CSS overrides.
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className={isMobile ? "bookingModal bookingModal--mobile" : "bookingModal"}
            role="dialog"
            aria-modal="true"
            aria-labelledby="bookingModalTitle"
            ref={modalRef}
            onClick={(event) => event.stopPropagation()}
            initial={isMobile ? { opacity: 0, y: 32 } : { opacity: 0, scale: 0.95, y: 16 }}
            animate={isMobile ? { opacity: 1, y: 0 } : { opacity: 1, scale: 1, y: 0 }}
            exit={isMobile ? { opacity: 0, y: 32 } : { opacity: 0, scale: 0.95, y: 16 }}
            transition={{ duration: isMobile ? 0.3 : 0.25, ease: EASE }}
          >
            {isMobile ? (
              <MobileBookingFlow
                ...
                isLeadFormComplete={isLeadFormComplete}
